...
const App: React.FC = () => {
  ...
  const handleGarmentSelect = useCallback(async (garmentFile: File, garmentInfo: WardrobeItem) => {
    ...
    const nextLayerInHistory = outfitHistory[currentOutfitIndex + 1];
    if (nextLayerInHistory && nextLayerInHistory.garment?.id === garmentInfo.id) {
  ...
  const handleSaveOutfit = () => {
    if (activeOutfitLayers.length <= 1 || !displayImageUrl) return;
  ...
  const handleLoadSavedOutfit = (saved: SavedOutfit) => {
    setOutfitHistory(saved.layers);
    setCurrentOutfitIndex(saved.layers.length - 1);
    setCurrentPoseIndex(0);
  };
